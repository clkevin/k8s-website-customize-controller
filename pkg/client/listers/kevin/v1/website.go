...
type WebsiteLister interface {
	...
	List(selector labels.Selector) (ret []*v1.Website, err error)
...
// List lists all Websites in the indexer.
func (s *websiteLister) List(selector labels.Selector) (ret []*v1.Website, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Website))
	})
	return ret, err
}
...
type WebsiteNamespaceLister interface {
	...
	Get(name string) (*v1.Website, error)
...
// List lists all Websites in the indexer for a given namespace.
func (s websiteNamespaceLister) List(selector labels.Selector) (ret []*v1.Website, err error) {
...
// Get retrieves the Website from the indexer for a given namespace and name.
func (s websiteNamespaceLister) Get(name string) (*v1.Website, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("website"), name)
	}
	return obj.(*v1.Website), nil
}
